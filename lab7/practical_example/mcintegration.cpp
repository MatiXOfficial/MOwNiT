...
{ 
    ...
    std::ofstream ofs; 
    std::string fileTitle = "./mcbeth/mcbeth" + std::to_string(npasses) + ".ppm";  // added
    ofs.open(fileTitle); 
    ofs << "P6\n" << width << " " << height << "\n255\n"; 
...
int main(int argc, char **argv) 
{
    ...
    if (argc != 2)
    {
        throw std::invalid_argument("Wrong number of arguments. Expecting ./mcintegration.cpp N");
    }
    ...
    while (npasses < N) {   // modified
        ...
        if (npasses == 1 || npasses == 10 || npasses == 50 || npasses == 100 || npasses == 200) // added
            raise(SIGINT); 
    } 
    delete [] pixels; 
    delete [] buffer;
